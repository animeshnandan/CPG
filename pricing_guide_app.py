...
def show_vehicle_popup():
    ...
    with right:
        details_df = pd.DataFrame({
            "Field": list(current_display.keys()),
            "Value": list(current_display.values())
        })
...
if vin_decoded:
    ...
    if vin_decoded.get("Trim") and "Trim" in filtered_df.columns:
        filtered_df = filtered_df[filtered_df["Trim"] == vin_decoded["Trim"]]
...
if selected_trims:
    filtered_df = filtered_df[filtered_df["Trim"].isin(selected_trims)]
...
if "Mileage" in filtered_df.columns:
    filtered_df = filtered_df[
        ...
        filtered_df["Mileage"].between(mileage_range[0], mileage_range[1], inclusive="both")
    ]

if sold_price_range is not None and "Sold Price" in filtered_df.columns:
    filtered_df = filtered_df[
        filtered_df["Sold Price"].isna() |
        filtered_df["Sold Price"].between(sold_price_range[0], sold_price_range[1], inclusive="both")
    ]
...
current_filter_state = {
    ...
    "mileage": tuple(st.session_state.get("filter_mileage", (0, 99999999))),
    "price": tuple(st.session_state.get("filter_price", (min_price, max_price))) if "Sold Price" in df.columns and df["Sold Price"].notna().any() else (),
...
m5.metric("Average B Price Trade-In", format_currency(avg_b_price_trade_in))
m6.metric("Average B Price Retail", format_currency(avg_b_price_retail))
